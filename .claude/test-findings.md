...
5. ~~**Planning label never removed**~~ — **FIXED (2026-02-16).** Added two steps to `claude.yml`: (a) detect approval keywords and remove planning label before run starts, (b) remove planning label after implementation completes. Fixes issue #20 where planning mode persisted through approval.
6. **"Claude encountered an error" message misleading** — When max-turns exhaustion occurs after work is completed, the GitHub comment header says "Claude encountered an error" even though all tasks were completed and code was committed. This message comes from the `anthropics/claude-code-action@v1` itself when the action hits max-turns. The work is still completed successfully (branch + commits exist), but the error header confuses users. **Status:** DOCUMENTED (not fixable in workflow; would require action update).
